...
* **History list** (`frontend/src/app/history/page.tsx`): Trash control on each card with `stopPropagation`; delete error toast on failure.
...
Demo run on cloud VM (2026-07-03, implementation at `bb479da`, demo at `c3bef16`). Full Docker stack (`postgres`, `api`, `frontend`, `backup` all Up). See `workflow/issues/issue-28/demo/demo-notes.md`.
...
| 4 — Failed delete shows error (optional) | **PASS** — Mocked 500 via Playwright route interception; destructive error toast shown; entry remained |
...
* **Scenario 4** — demonstrated with mocked 500 (Playwright route interception) rather than stopping the API container, so the stack stayed healthy for remaining checks.

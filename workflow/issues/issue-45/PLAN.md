...
| `docker-compose.yml` | **Modify** | Add `backup` service: `depends_on: postgres`, mount `./backups`, env vars |
| `.gitignore` | **Modify** | Ignore `backups/` (generated dumps) |
| `backups/.gitkeep` | **Add** | Preserve directory in git without committing dumps |
...
2. Add `backups/.gitkeep` so the mount path exists on fresh clones.
...
4. **AGENTS.md:** mention fourth container in stack verification; note `./backups/` retention.
...
| Unique dated filenames in `./backups/` | **Demo Scenario 2:** file matches `cuebox-YYYY-MM-DD.dump` pattern |
...
| `backups/` in `.gitignore` | **Execute:** `git check-ignore -v backups/cuebox-test.dump` returns match |
...
| Cloud agent `./backups/` on bind mount | Retention caps at 2 files; negligible footprint |
...
**Rollback:** remove `backup` service from `docker-compose.yml`, delete `backup/` image context, revert scripts; existing data unaffected. Remove `./backups/` manually if desired.
...
- [ ] `./backups/` host mount receives dated `.dump` files; `backups/` gitignored

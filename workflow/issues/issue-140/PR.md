...
* Updated `frontend/src/components/library-search-picker.tsx` — remove `intent` / `SearchPickerIntent`; unified **Find a film…**; optional `autoFocus`; TMDB **Add & mark watched** with enrichment poll before status transition; post-review stay + refetch
...
* Added AppShell header magnifying-glass link (**Search films** → `/search`) in `frontend/src/components/app-shell.tsx`
...
Demo notes: optional Add & mark watched click-through video not captured (buttons proven live; mocked E2E covers enrichment → review dialog). Scenario 5 skipped to keep the seeded volume.
...
6. From `/watchlist`, use header **Search films** (magnifying glass) — lands on Home picker via `/search`.
7. Search a TMDB-only title (requires `TMDB_API_KEY`) — confirm **Add to watchlist** and **Add & mark watched**; mark-watched opens review dialog after enrichment is ready.
8. Optional empty-hub check: empty watchlist + `/?focus=search` shows Import CTA only (focus no-op).
9. Run Phase 6 gates:
   ```bash
...
* Draft PR #147 already exists — do not open a new PR.

...
def test_reject_on_enriching_film_returns_409(integration_client):
    """accept_flag match: film is enriching but no longer review_required."""
    suffix = uuid.uuid4().hex[:8]
    uri = f"https://letterboxd.com/film/enriching-{suffix}/"
    csv_content = (
        "Date,Title,Year,Letterboxd URI\n"
        # Year off by one → accept_flag (enriching + pending review, not review_required)
    ...
    assert film["enrichment_status"] == "enriching"
    ...
    assert refreshed["enrichment_status"] == "enriching"
